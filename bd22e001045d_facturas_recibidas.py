...
Revision ID: bd22e001045d
Revises: 9b3628f270e4
Create Date: 2026-09-10 22:54:43.618171
...
revision: str = 'bd22e001045d'
down_revision: Union[str, Sequence[str], None] = '9b3628f270e4'
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None


"""facturas recibidas del SAT

Crea la tabla FacturasRecibidas.

SolicitudesSAT NO se modifica: ya tiene `tipo_solicitud` (metadata|cfdi) y
`tipo_comprobante` (emitidos|recibidos). Para las recibidas basta con
escribir "recibidos" en `tipo_comprobante` al crear la solicitud.

Revision ID: RELLENAR
Revises: RELLENAR
Create Date: RELLENAR
"""
from alembic import op
import sqlalchemy as sa


# TODO: conservar los IDs que genere `python -m alembic revision -m "..."`
revision = "RELLENAR"
down_revision = "RELLENAR"
branch_labels = None
depends_on = None
...
def downgrade():
    # Orden inverso exacto
    op.drop_constraint("ck_SolicitudesSAT_tipo_descarga", "SolicitudesSAT", type_="check")
    op.drop_constraint("ck_SolicitudesSAT_tipo_solicitud", "SolicitudesSAT", type_="check")
    op.drop_column("SolicitudesSAT", "tipo_descarga")
    op.drop_column("SolicitudesSAT", "tipo_solicitud")
